...
counts.list.wf.M71 <- counts.list.wf$FACSseq_M71_20170405[,1:3]
counts.list.wf.MOR23 <- counts.list.wf$FACSseq_MOR23_20170112[,4:6]
counts.list.wf.exposed <- cbind(counts.list.wf.M71,counts.list.wf.MOR23)
exp_table_exposed <- "exposed_Sample_list.txt"

'%ni%' <- Negate('%in%')
spikesexposed <- counts.list.wf.exposed[grep("ERCC", rownames(counts.list.wf.exposed)),]
exposed.data <- counts.list.wf.exposed[rownames(counts.list.wf.exposed) %ni% rownames(spikesexposed),]





s2cexposed <- read.table(exp_table_exposed,
                       header = TRUE,
                       stringsAsFactors = FALSE)
rownames(s2cexposed) <- s2cexposed$sample


Olfr151 <- exposed.data[grep("Olfr151", rownames(exposed.data)),]
Olfr151 <- Olfr151[1,]

Olfr16 <- exposed.data[grep("Olfr16", rownames(exposed.data)),]
Olfr16 <- Olfr16[1,]

Olfr.genes <- exposed.data[grep("Olfr", rownames(exposed.data)),]

exposed.data <- exposed.data[rownames(exposed.data) %ni% rownames(Olfr.genes),]


exposed.data <- rbind(exposed.data,Olfr151)
exposed.data <- rbind(exposed.data, Olfr16)

filterthresholdexposed<- GetCountThreshold_generic(counts=exposed.data,
                                               return.plot = TRUE)
genes.to.keep.exposed <- rowSums(x = exposed.data) != 0
counts.filtered.exposed <- exposed.data[genes.to.keep.exposed,]

# Normalize the data to determine threshold of gene expression
norm.counts.exposed <- (t(x = t(x = counts.filtered.exposed) / colSums(x = counts.filtered.exposed)) * 1e6)




filtered.exposed <- FilterCountsMatrix(counts=norm.counts.exposed,
                                  condition.1.names = s2cexposed$sample[1:3],
                                  condition.2.names = s2cexposed$sample[4:6],
                                  count.threshold = filterthresholdexposed$count.threshold)



genes.filtered.exposed <- rownames(filtered.exposed$filtered.counts)


exposed.data <- exposed.data[rownames(exposed.data) %in% genes.filtered.exposed,]

ddsexposed <- ImportCountsMatrixDESeq2(counts = exposed.data, 
                                     colData = s2cexposed,
                                     condition.levels = c("M71",
                                                          "MOR23"))

resexposed <- RunDESeq2(dds = ddsexposed, 
                    #                    normGenes = filteredM71$genes, 
                    #   genestokeep = filteredM71$genes,
                    colcontrolsamples = s2cexposed$sample[1:3],
                    colexposedsamples = s2cexposed$sample[4:6],
                    coef = "condition_MOR23_vs_M71")


#Data visualization
#PCA
GetPCAplot(dds = resexposed$dds,
           intgroup = "condition")
...
resexposedmutatedLFC <- MutateCountsdfLFC(res = resexposed$resLFCMerged,
                                      threshold.value = 0.1)
...
#M71
x <-GetScatterplot(Mutated.df =  resexposedmutatedLFC,
...
#VolcanoPlot
GetVolcanoplot(Mutated.df = resexposedmutatedLFC,
               threshold = 0.1,
               inf.limit = -11,
               sup.limit = 11,
               OlfrOfInterest = "Olfr151",
