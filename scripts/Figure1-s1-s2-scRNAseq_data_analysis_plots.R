...
source("S:/IRlab-Server/_Scripts/R_lib/center.palette.R") # from https://github.com/AmyOlex/RNASeqBits/blob/master/R/center.palette.R
...
matrix.moe$clusters <- factor(x = moe.filter.sct.integrated@active.ident,levels = c("Sus",
                                                                                    ...
                                                                                    "GBC"))
# Violin plot MOE
moe.vlnplot <- lapply(X = colnames(matrix.moe)[-14],
                      FUN = function(gene){
...
# Read the OSN data 
osn.no.or.sub.integrated.sct <- read_rds("moe_noexp_scRNAseq10x_cellranger_filter_OSN_NO_OLFR_sct_integrated_clusters.rds")
...
olfr.clusters <- wrap_plots(olfr.clusters,ncol = 2)
...
olfr.data <- as.data.frame(t(olfr.data[genes.1,]))
...
top.6.olfr.ventral <- lapply(osn.split.by.cluster.metadata, FUN = function(x){
  a <- names(head(sort(table(x$olfr.ident),decreasing = T),6))
  
})
top.6.olfr.ventral <- unname(unlist(top.6.olfr.ventral[2]))
...
heatmap.inter <- heat_map_plot(olfr.vln,"all")

heatmap.intra <- heat_map_plot(top.6.olfr.ventral,"Ventral")
...
# NMI score
osn.sub.integrated.sct <- read_rds("moe_noexp_scRNAseq10x_cellranger_filter_OSN_WITH_OLFR_sct_integrated_clusters.rds")
...
olfr.clusters.or <- wrap_plots(olfr.clusters.or,ncol = 3)

sup.fig.2 <- wrap_plots(osn.dimplot,
                        olfr.clusters,
                        osn.dimplot.or,
                        olfr.clusters.or,
                        ncol = 2)

ggsave(filename = "sup.fig.2.pdf",
       plot = sup.fig.2,
       device = "pdf",
       width = 10,height = 10)
