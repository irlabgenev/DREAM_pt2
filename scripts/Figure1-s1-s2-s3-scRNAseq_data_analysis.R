## Main olfactory epithelium data filtering ##

# This script is used to filter and cluster identifiaction/annotation of the MOE and mOSN cells. 
# It generates various Seurat objects used for the plots.
...
moe.integrated.sct <- RunUMAP(object = moe.integrated.sct, dims = 1:9 )

# Select clusters to remove:
Immune.cells <- c(7,9,16,19,24,25,26) # based on a the expression level of Igkc, Cd52, Cybb, Ctss,Tyrobp
Apoptotic.cells <- c(15,18,11,14) # based on a high percent.mt and low nFeatures
Blood.cells <- c(17) # based on a the expression level of Hbb-a1 and Hbb-a2
...
Clusters.to.keep <- as.vector(as.numeric(unique(levels(moe.integrated.sct$integrated_snn_res.1))))
...
moe.filter.sct.integrated <- RenameIdents(object = moe.filter.sct.integrated,
                                          ...
                                          "5" = "iOSN1", # Markers Gap43, Gng8
                                          "6" = "iOSN2", # Markers Gap43, Omp
                                          "7" = "HBC2", # Markers Trp63, Krt5
                                          "8" = "mOSN", # Markers Omp, Cnga2, Gnga13
...
#saveRDS(object = moe.filter.sct.integrated,
#        file = "moe_noexp_scRNAseq10x_cellranger_filter_sct_integrated_clusters.rds")
...
#saveRDS(object = osn.data.sub,
#        file = "moe_noexp_scRNAseq10x_cellranger_filter_OSN.rds")
...
osn.no.or.sub.integrated.sct <- read_rds(file = "moe_noexp_scRNAseq10x_cellranger_filter_OSN_NO_OLFR_sct_integrated_clusters.rds")
...
x <- osn.sub.integrated.sct@reductions$umap@cell.embeddings

osn.sub.integrated.sct@reductions$umap@cell.embeddings[,1] <- ifelse(x[,1] < 0,
                                                                     yes = abs(x[,1]),
                                                                     no = -x[,1])

osn.sub.integrated.sct@reductions$umap@cell.embeddings[,2] <- ifelse(x[,2] < 0,
                                                                     yes = abs(x[,2]),
                                                                     no = -x[,2])
